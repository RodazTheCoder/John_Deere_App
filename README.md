...
**Interruptor "Modo Demo"**: com hardware real conectado (ESP32 de verdade transmitindo por LoRa), dado real e dado de demo podiam entrar em conflito (ex: uma tag desconectada ficava "grudada" no último valor, competindo com os botões). Agora existe um botão `MODO DEMO: LIGADO/DESLIGADO` que decide qual fonte conta — nunca mistura os dois (`POST /api/modo-demo`, filtrado em `app.py > _alerta_atual()`). Também foi corrigido o bug de entidade "grudada": `estado.ler_entidades()` agora aceita um timeout (`config.ENTIDADE_TIMEOUT_S = 30`) e esconde entidades que pararam de atualizar (ex: tag desligada), em vez de manter o último valor pra sempre.

**Escala configurável**: os limiares verde/amarelo (padrão 100m/50m) podem ser reconfigurados em tempo real pelo dashboard (`POST /api/escala`, campo de texto no rodapé — aceita qualquer valor, o amarelo é sempre metade do verde). Serve pra testar com hardware real em ambientes pequenos (sala, escritório) sem precisar alcançar 100m de verdade — os mesmos poucos metros de separação entre as placas já atravessam as três zonas (verde/amarelo/vermelho). Vale tanto pra dado real quanto pros botões de demo; reseta pro padrão de `config.py` com `{"verde_m": null}`.

**A lógica de decisão continua no ESP32 quando o Pi cai** (redundância física, decisão de arquitetura desde o início do projeto) — isso não mudou com a escala configurável. `/api/alerta-fisico` agora manda também os dois limiares atuais (`cor,piscando,som,verde_m,amarelo_m`), e o ESP32 guarda isso como cache (`DISTANCIA_VERDE_M`/`DISTANCIA_AMARELO_M` deixaram de ser `const` no firmware). Se o Pi ficar inacessível, o fallback local usa a **última escala confirmada**, não sempre o padrão de fábrica de 100m — importante pra não "voltar pra 100m" no meio de um teste em sala se o Wi-Fi oscilar.
